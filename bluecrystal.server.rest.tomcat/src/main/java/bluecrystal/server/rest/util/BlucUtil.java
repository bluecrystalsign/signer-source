package bluecrystal.server.rest.util;
...
public class BlucUtil {
	...
	public boolean validarECompletarPacoteAssinavel(byte[] certificado, byte[] sha1,
	...
	public boolean produzPacoteAssinavel(byte[] certificado, byte[] sha1,
	...
	public int validateSign(byte[] assinatura, byte[] sha1, byte[] sha256,
		...
		if (politica == null) {
			int signOk = ccServ.validateSign(assinatura, sha1, dtAssinatura,
					verificarLCRs);
			...
			int signOk = ccServ.validateSign(assinatura, origHash, dtAssinatura,
					verificarLCRs);
			if (signOk != StatusConst.GOOD) {
				resp.setError("Não foi possível validar a assinatura digital");
				return signOk;
